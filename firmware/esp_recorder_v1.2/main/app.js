...
(function() {
        ...
        switch (path) {
          case '/api/status':
            resp = {
              ...
              sd: true, sd_free: '7.2 GB', sd_usb_owned: false,
              file_api_available: true,
              recording: recStartTime > 0,
              file_open: recStartTime > 0,
              current_file: recStartTime > 0 ? 'REC_CH1_20260724_143022.bin' : '-',
              rec_drop_count: 0, rec_total_bytes: recBytes,
              stream_gap_ms: streamGapMs,
              uart_baudrate: 115200,
              ch1_drop_count: 0, ch2_drop_count: 0,
              ch1_total_bytes: recBytes, ch2_total_bytes: 0,
              ch1_file_open: recStartTime > 0, ch2_file_open: false,
              uart_ch1_ready: true, uart_ch2_ready: true,
              uart_ch1_rx_bytes: rxBytes, uart_ch2_rx_bytes: 0,
              uart_ch1_tx_bytes: txBytes, uart_ch2_tx_bytes: 0,
              route_enabled: true,
              route_ch1_to_ch2_bytes: 0, route_ch2_to_ch1_bytes: 0,
              route_ch1_to_ch2_drop_bytes: 0, route_ch2_to_ch1_drop_bytes: 0,
              uptime: runningSec + Math.floor(Math.random() * 5),
              free_heap: 245760 + Math.floor(Math.random() * 50000),
              fw_version: 'v1.2.0', usb_mode: 'CDC+MSC',
  ...
  async function refreshStatus() {
    ...
    setStatusEl('st-baud', (s.uart_baudrate || 115200) + ' bps');
    ...
    setStatusEl('st-usb', s.usb_mode || 'CDC+MSC');
    setStatusEl('st-srv', s.ap ? '已启动' : '未启动', s.ap ? 'on' : 'off');
    setStatusEl('st-uptime', formatUptime(s.uptime || runningSec));
    setStatusEl('st-heap', formatBytes(s.free_heap || 0));
    setStatusEl('st-fw', s.fw_version || 'v1.2.0');
    /* 更新记录控制区 */
  ...
  function initUartConfig() {
    document.getElementById('btn-uart-save').addEventListener('click', async function() {
      ...
      var parityStr = document.getElementById('uart-parity').value;
      var parity = (parityStr === 'odd') ? 1 : (parityStr === 'even') ? 2 : 0;
      if (IS_LOCAL) {
        toast('UART 配置已保存（模拟）: ' + baud + ' ' + bits + bitsToStr(bits) + stopToStr(stop) + '-' + parityStr);
        return;
      }
      var r = await api('/api/config/uart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
