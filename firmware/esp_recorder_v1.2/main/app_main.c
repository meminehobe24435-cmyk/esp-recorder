...
void app_main(void)
{
    ...
    /* SD 卡可能未插入 — 不影响 WiFi/Web 功能，仅记录状态。 */
    if (sd_card_init() != ESP_OK) {
        ESP_LOGW(TAG, "SD card not available — WiFi & Web will still work, recording disabled");
    }

    /* USB CDC + MSC：失败不阻止启动（WiFi 优先）。 */
    if (usb_cdc_init(usb_cdc_rx_handler) != ESP_OK) {
        ESP_LOGW(TAG, "USB CDC init failed, continuing without USB serial");
    }
    if (usb_msc_init() != ESP_OK) {
        ESP_LOGW(TAG, "USB MSC init failed, continuing without USB storage");
    }

    /* 加载配置（NVS 优先，SD 镜像兜底；无 SD 则只用 NVS + 默认值）。 */
    ...
    if (recorder_init() != ESP_OK) {
        ESP_LOGW(TAG, "recorder init failed, continuing without recording");
    }
    ...
    /* ===== WiFi + Web 是核心功能，失败记录日志但不中止 ===== */
    if (wifi_config_init() != ESP_OK) {
        ESP_LOGE(TAG, "WiFi init failed");
    }
    if (wifi_apply_ap(cfg->wifi.ap_ssid, cfg->wifi.ap_pass, cfg->wifi.ap_ip) != ESP_OK) {
    ...
    if (cfg->recorder.auto_start)
    {
        ...
        {
            ESP_LOGW(TAG, "auto start recording failed (no SD card?)");
        }
    }
    else
    {
