...
/* 设备配置以 NVS 为主存储，SD 卡 config.json 为可读镜像。 */
typedef struct {
    struct {
    ...
    struct {
        bool enabled;         /* 默认 true：CH1 RX <-> CH2 RX 双向交叉路由 */
    } router;
} device_config_t;
...
bool config_manager_valid_baudrate(int v);

/* 设置 stream_gap_ms（先 clamp，再写内存，最后落盘；失败回滚） */
esp_err_t config_manager_set_stream_gap_ms(config_manager_t* manager, int v,
                                          const char* save_path);
esp_err_t config_manager_set_uart_baudrate(config_manager_t* manager, int v,
                                           const char* save_path);
esp_err_t config_manager_set_router_enabled(config_manager_t* manager, bool enabled,
                                            const char* save_path);
esp_err_t config_manager_set_recorder_auto_start(config_manager_t* manager, bool enabled,
                                                 const char* save_path);
