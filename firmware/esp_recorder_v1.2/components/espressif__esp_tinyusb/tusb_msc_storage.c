...
static char cUSBSwitchFlag = 1;
void WT_SetUSBWriteSwitch(char cSW)
{
    cUSBSwitchFlag = cSW;
}
...
static esp_err_t _read_sector_sdmmc(size_t sector_size,
                                    ...
                                    void *dest)
{
    if(cUSBSwitchFlag==0) return ESP_ERR_INVALID_STATE;
...
esp_err_t tinyusb_msc_storage_mount(const char *base_path)
{
    ...
    ESP_LOGI(TAG, "tinyusb_msc_storage_mount");
    // connect driver to FATFS
    
    ESP_RETURN_ON_ERROR(ff_diskio_get_drive(&drv), TAG,
                        "The maximum count of volumes is already mounted");
    char pdrv[3] = {(char)('0' + drv), ':', 0};

    ESP_GOTO_ON_ERROR((s_storage_handle->mount)(drv), fail, TAG, "Failed pdrv=%d", drv);

    // FATFS *fs = NULL;
    ret = esp_vfs_fat_register(base_path, pdrv, s_storage_handle->max_files, &fs);
    ...
    ESP_GOTO_ON_ERROR(_mount(pdrv, fs), fail, TAG, "Failed _mount");
...
esp_err_t tinyusb_msc_storage_unmount(void)
{
    ...
    ESP_LOGI(TAG, "tinyusb_msc_storage_unmount");
    tusb_msc_callback_t cb = s_storage_handle->callback_premount_changed;
    if (cb) {
        tinyusb_msc_event_t event = {
            .type = TINYUSB_MSC_EVENT_PREMOUNT_CHANGED,
