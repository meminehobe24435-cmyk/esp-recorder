...
void app_main(void)
{
    ...
    if (cfg_err != ESP_OK)
    {
        ESP_LOGW(TAG, "configuration not found, saving defaults");
        config_manager_reset_defaults(cfg_mgr);
        ESP_ERROR_CHECK(config_manager_save(cfg_mgr, "/sdcard/config.json"));
    }
    ...
    ESP_ERROR_CHECK(data_router_init(cfg->router.enabled));
    ...
    /* USB MSC starts last so mount callbacks see initialized recorder/config. */
